...
spec = describe "ByteBuffer" $ do
    it "can grow to store a value and return it." $ BB.withByteBuffer 0 $ \ bb -> do
        let bs = "some bytestring"
        BB.copyByteString bb bs
        bs' <- BB.consume bb (BS.length bs)
        bs' `shouldBe` Right bs
        bbIsEmpty bb
    it "should request more input when needed." $ BB.withByteBuffer 0 $ \ bb -> do
        let bs = "some bytestring"
        BB.copyByteString bb bs
    ...
    it "should not grow if bytes can be freed." $
        ...
        in BB.withByteBuffer (BS.length bs1) $ \ bb -> do
