...
import           GHC.Generics
import           GHC.Prim ( unsafeCoerce#, RealWorld, State#, Addr#, readWord8OffAddr#, Int# , (+#))
...
peekStorable = error "peekStorable"

peekWord8 :: Peek Word8
peekWord8 = Peek (\_FIXME addr off s0 ->
    case readWord8OffAddr# addr off s0 of
        (# s1, w #) -> (# s1, off +# 1#, W8# w #))
{-# INLINE peekWord8 #-}
...
repPeek_ :: Int -> Peek a -> Peek ()
repPeek_ cnt0 (Peek f) =
    Peek (\total ptr off0 s0 ->
        let loop 0 off1 s1 = (# s1, off1, () #)
            loop cnt off1 s1 =
                case f total ptr off1 s1 of
                    (# s2, off2, _ #) -> loop (cnt - 1) off2 s2
         in loop cnt0 off0 s0)
...
instance Applicative Peek where
    pure x = Peek (\_ _ offset s -> (# s, offset, x #))
    {-# INLINE pure #-}
    (<*>) = ap
    {-# INLINE (<*>) #-}
...
instance Monad Peek where
    return = pure
    {-# INLINE return #-}
    Peek f >> Peek g = Peek (\total ptr offset1 s1 ->
        case f total ptr offset1 s1 of
            (# s2, offset2, _ #) -> g total ptr offset2 s2)
    {-# INLINE (>>) #-}
    Peek f >>= g = Peek (\total ptr offset1 s1 ->
        case f total ptr offset1 s1 of
            (# s2, offset2, x #) -> runPeek (g x) total ptr offset2 s2)
    {-# INLINE (>>=) #-}
...
peekException _ = error "peekException" -- msg = Peek $ \_ _ offset _ -> throwIO (PeekException offset msg)
